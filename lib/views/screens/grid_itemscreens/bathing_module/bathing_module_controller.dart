// bathing_module_controller.dart
import 'package:get/get.dart';
import 'package:autism_fyp/views/controllers/progress_controller.dart';

class BathingModuleController extends GetxController {
  final progressController = Get.find<ProgressController>();

  final String moduleId = "bathing";

  var quizResults = <String, Map<String, dynamic>>{}.obs;
  var wrongAnswers = <String, List<Map<String, dynamic>>>{}.obs; // Track wrong answers per quiz
  ...
  Future<void> syncModuleProgress() async {
    for (var entry in quizResults.entries) {
      ...
      try {
        await progressController.updateQuizProgress(
          ...
          wrongAnswers: result["wrongAnswers"] ?? 0, // Pass wrong answers count
        );
        
        // Sync individual wrong answers
        if (wrongAnswers.containsKey(quizId)) {
    ...
    // Clear wrong answers after syncing
    wrongAnswers.clear();
  }
}